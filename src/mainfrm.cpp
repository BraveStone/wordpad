...
static const UINT TEXT_COLOR_TEMPLATE_INDEX = 8;
...
{
	TRACE(_T("in CMainFrame::CMainFrame\n"));
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// set initial color for color button
	m_wndFormatBar.SetColorButton(ID_SET_TEXT_COLOR, RGB(0,0,0));
...
void CMainFrame::OnTextColor()
{
	...
	if (pView)
	{
		// get color of current selection
		CRichEditCtrl& richedit = pView->GetRichEditCtrl();
		CHARFORMAT cf;
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		// set new text color in rich edit view
		if (pView)
			pView->SetColor(rgb);
	}
}
